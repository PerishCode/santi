...
async fn labeled_turn_emits_envelope_and_records_outbox() {
    ...
    let santi_core::ingest::Outcome::Accepted { .. } = service
        .evented(&soul, label, "external request".to_string())
        .expect("ingest event")
    else {
...
async fn downstream_batch_isolates_zone_and_advances_over_other_zones() {
    ...
    .expect("open service");
    let soul = service.souls().expect("list souls")[0].id.clone();
    let mut stream = service.listen();
    service
        .evented(&soul, "github:issue:1", "github".to_string())
    ...
    service
        .evented(&soul, "stim:alice", "stim".to_string())
        .expect("ingest stim");
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            if let santi_core::stream::Payload::TurnCompleted {
