...
mod tests {
    ...
    async fn spawn_counting_http_server(events_response: FakeEventsResponse) -> CountingHttpServer {
        ...
        tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    handle_fake_http_request(stream, post_count, get_count, events_response).await;
                });
            }
        });
        CountingHttpServer {
    ...
    async fn handle_fake_http_request(
    ...
    #[tokio::test]
    async fn strand_send_watch_posts_once_when_watch_completes() {
        let server = spawn_counting_http_server(FakeEventsResponse::CompletesSeedTurn).await;
    ...
    #[tokio::test]
    async fn strand_send_watch_posts_once_when_event_stream_closes() {
        let server = spawn_counting_http_server(FakeEventsResponse::ClosesImmediately).await;
        let client = reqwest::Client::new();
        ...
        assert_eq!(
            server.post_send_count.load(Ordering::SeqCst),
            1,
            "watch stream close must not reconnect by resubmitting the send POST"
        );
        assert_eq!(server.get_events_count.load(Ordering::SeqCst), 1);
    }

    #[tokio::test]
    async fn strand_send_watch_posts_once_when_watch_request_fails() {
        let server = spawn_counting_http_server(FakeEventsResponse::Status500).await;
        let client = reqwest::Client::new();
        ...
        assert!(
            error.to_string().contains("request failed with status"),
            "unexpected error: {error:#}"
        );
        assert_eq!(
            server.post_send_count.load(Ordering::SeqCst),
            1,
            "watch/follow failure must not resubmit the send POST"
        );
        assert_eq!(server.get_events_count.load(Ordering::SeqCst), 1);
    }
