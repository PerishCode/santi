...
fn register(service: &Service, id: &str, prefix: &str, token: &str) {
    service
        .enroll(santi_core::downstream::Draft {
            id: id.to_string(),
...
async fn credential_and_zone_gate_ingest() {
    ...
    assert!(matches!(
        service
            .downstream("s3cret", request.clone())
            .expect("ingest"),
        service::Admission::Accepted(_)
    ));
    assert!(matches!(
        service
            .downstream("wrong-token", request.clone())
            .expect("ingest"),
...
async fn request_key_replays_receipt_and_rejects_changed_payload() {
    ...
    let strand_count = service.strands().expect("list strands").len();
    let error = match service.downstream(
        "s3cret",
        santi_core::ingest::Request {
    ...
    assert_eq!(service.strands().expect("list strands").len(), strand_count);
}
...
fn registration_is_unique_idempotent_and_secret_free() {
    ...
    let created = service.enroll(request.clone()).expect("create downstream");
    let repeated = service.enroll(request).expect("repeat registration");
