...
export async function deploy(argv: string[]): Promise<number> {
  ...
  const remote = [
    ...
    `DURABLE_PREVIOUS=${SANTI_HOME}/runtime/upgrade/installed-package.json`,
    ...
    'if [ -r "$DURABLE_PREVIOUS" ]; then',
    '  echo ">> durable rollback manifest present"',
    "else",
